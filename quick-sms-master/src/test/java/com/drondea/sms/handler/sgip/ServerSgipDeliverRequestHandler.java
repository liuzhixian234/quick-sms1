...
public class ServerSgipDeliverRequestHandler extends SimpleChannelInboundHandler<SgipDeliverRequestMessage> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, SgipDeliverRequestMessage msg) throws Exception {
        ...
        // 在 ServerSgipDeliverRequestHandler 的 channelRead0 方法开头添加
        System.out.println("ServerSgipDeliverRequestHandler 接收到消息: " + msg.getClass().getName());
        System.out.println("=== 收到上行短信（System.out）==="); // 使用System.out确保输出
        log.info("收到上行短信：{}", msg.toString());
